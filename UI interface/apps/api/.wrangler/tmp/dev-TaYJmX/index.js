...
app.get("/api/dashboard/summary", async (c) => {
  ...
  const [jobCount] = await query(env, "SELECT COUNT(*)::text AS count FROM jobs WHERE user_id = $1", [userId]);
...
var JOBS_SORT_COLUMNS = ["date_saved", "company", "role", "referral_status", "job_link"];
...
app.get("/api/jobs", async (c) => {
  ...
  if (company) {
    ...
    return c.json({ page, limit, data: rows2 });
  }
...
app.get("/api/referrals", async (c) => {
  ...
  const sql = filterOpen ? "SELECT * FROM referrals WHERE user_id = $1 AND COALESCE(TRIM(referral_received), '') IN ('Requested', 'Pending') ORDER BY request_date DESC NULLS LAST, id DESC LIMIT $2 OFFSET $3" : "SELECT * FROM referrals WHERE user_id = $1 ORDER BY request_date DESC NULLS LAST, id DESC LIMIT $2 OFFSET $3";
...
app.patch("/api/referrals/:id", async (c) => {
  ...
  if (p2.referral_received !== void 0) {
    ...
    values.push(p2.referral_received);
  }
...
app.post("/api/notes", async (c) => {
  ...
  const [row] = await query(
    ...
    [userId, p2.note_date ?? null, p2.comments]
  );
  return c.json(row, 201);
});
...
var pendingPatchInput = external_exports.object({ is_done: external_exports.literal(true) });
...
// .wrangler/tmp/bundle-k9ANKj/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-k9ANKj/middleware-loader.entry.ts
